...
class JsonizeDataVecFloat(object):
    ...
    导入：from unitree_sdk2_cpp.robot.h1 import JsonizeDataVecFloat
    构造可用性：AVAILABLE。
    """
...
class JsonizeTargetPos(object):
    ...
    导入：from unitree_sdk2_cpp.robot.h1 import JsonizeTargetPos
    构造可用性：AVAILABLE。
    """
    def __init__(self) -> None:
        ...
        ...
    x: float
    """JsonizeTargetPos.x：公开字段，类型为 float。
    具体单位及允许值以对应 SDK 数据结构为准。
    """
    y: float
    """JsonizeTargetPos.y：公开字段，类型为 float。
    具体单位及允许值以对应 SDK 数据结构为准。
    """
    yaw: float
    """JsonizeTargetPos.yaw：公开字段，类型为 float。
    具体单位及允许值以对应 SDK 数据结构为准。
    """
    relative: bool
    """JsonizeTargetPos.relative：公开字段，类型为 bool。
    具体单位及允许值以对应 SDK 数据结构为准。
    """
    def from_json(self, value: Mapping[str, Any]) -> None:
        """从 JSON 风格字典读取字段并更新当前 SDK 值对象。
...
class JsonizeVelocityCommand(object):
    ...
    导入：from unitree_sdk2_cpp.robot.h1 import JsonizeVelocityCommand
    构造可用性：AVAILABLE。
    """
    def __init__(self) -> None:
...
class LocoClient(Client):
    ...
    导入：from unitree_sdk2_cpp.robot.h1 import LocoClient
    构造可用性：AVAILABLE。
    """
    def __init__(self) -> None:
    ...
    def set_phase(self, phase: Sequence[float]) -> int:
        ...
        Args:
            phase: 传给该接口的 相位 参数，Python 类型为 `Sequence[float]`。精确含义、单位、范围和枚举值需查目标型号对应头文件与协议。 对应 C++ 参数 `phase: std::vector<float>`。 底层是可变长度 vector；元素约束：底层为浮点数；类型本身不说明单位、坐标系或业务安全范围。
    ...
    def set_target_pos(self, x: float, y: float, yaw: float, relative: bool = True) -> int:
        ...
        Args:
            ...
            relative: 传给该接口的 `relative` 参数，Python 类型为 `bool`。精确含义、单位、范围和枚举值需查目标型号对应头文件与协议。 对应 C++ 参数 `relative: bool`。 只接受布尔语义。
        ...
        Notes:
            会向实体机器人发送运动请求；仅在确认目标状态并准备停止手段后调用。
            C++: SetTargetPos(float, float, float, bool)
        """
        ...
    ...
    def set_next_foot(self, foot: bool) -> int:
        ...
        Args:
            foot: 传给该接口的 `foot` 参数，Python 类型为 `bool`。精确含义、单位、范围和枚举值需查目标型号对应头文件与协议。 对应 C++ 参数 `foot: bool`。 只接受布尔语义。
